...
def _call_openai(
    ...
    *,
    ...
    if not json_payload and "output_json" in data and isinstance(data.get("output_json"), dict):
...
def _coerce_resume_json(data: Any) -> Dict[str, Any]:
    """Make a best-effort to coerce arbitrary data into our expected resume JSON shape."""
    result: Dict[str, Any] = {}
    ...
    result["skills"] = [s for s in skills if isinstance(s, (str, int, float))]
    result["skills"] = [str(s) for s in result["skills"]]

    # experience (list[dict]) with bullets list
    ...
    for e in exp_list:
        ...
        item["bullets"] = [b for b in bullets if isinstance(b, (str, int, float))]
        item["bullets"] = [str(b) for b in item["bullets"]]
