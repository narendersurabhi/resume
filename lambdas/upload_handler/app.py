...
def _ok(body, status=200):
    return {"statusCode": status, "headers": _cors_headers(origin), "body": json.dumps(body)}

def _err(status, msg):
    return {"statusCode": status, "headers": _cors_headers(origin), "body": json.dumps({"error": msg})}
